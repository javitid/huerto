...
const EMPTY_DASHBOARD_VIEW_MODEL: DashboardViewModel = {
  pendingTasks: 0,
  inProgressTasks: 0,
  completedTasks: 0,
  tasks: []
...
export class DashboardFacade {
  ...
  readonly showInvoiceAnalysis = signal(true);
  ...
  constructor(
    ...
    private readonly dashboardFirestore: DashboardFirestoreService,
    private readonly dashboardFileAnalysis: DashboardFileAnalysisService,
    ...
    this.dashboardData$ = this.user$.pipe(
      ...
      catchError(() => of(EMPTY_DASHBOARD_VIEW_MODEL))
    );
  }
  ...
  setTaskStatus(status: string): void {
    this.taskStatus.set(this.normalizeTaskStatus(status));
  }
